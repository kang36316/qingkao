...
class Index extends \think\Controller {
	...
	public function _initialize() {
		...
		if (request()->action() != 'complete' && is_file(APP_PATH . 'database.php') && is_file(  'data/install.lock')) {
	...
	public function check() {
        ...
        $this->status['error'] =session('error');
		$this->assign('status', $this->status);
		return $this->fetch();
	}
	...
	public function config($db = null, $admin = null) {
        if(session('error')){
            return $this->redirect(url('install/index/check'));
            exit();
        }
		if (request()->IsPost()) {
			//检测管理员信息
