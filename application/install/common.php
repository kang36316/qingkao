...
function check_dirfile(){
	$items = array(
		...
		array('dir',  '可写', 'success', 'public/upload/cache'),
		array('dir',  '可写', 'success', 'public/upload/file'),
		array('dir',  '可写', 'success', 'public/upload/image'),
		array('dir',  '可写', 'success', 'public/upload/qrcode'),
        array('dir',  '可写', 'success', 'public/update/download'),
        array('dir',  '可写', 'success', 'public/update/unzip'),
	);
...
function write_config($config){
	if(is_array($config)){
		//读取配置内容
		$conf = file_get_contents('data/db.tpl');
		//替换配置项
...
function create_tables($db, $prefix = ''){
	//读取SQL文件
	$sql = file_get_contents( 'data/sql.sql');
	$sql = str_replace("\r", "\n", $sql);
	$sql = explode(";\n", $sql);
	//替换表前缀
	$orginal = 'edu_';
...
function register_administrator($db, $prefix, $admin){
	...
	$sql = "INSERT INTO `[PREFIX]admin` (`id`,`username`,`password`, `login_count`,`create_time`,`last_login_ip`,`last_login_time`,`status`) VALUES ".
		   "('1', '[NAME]', '[PASS]', '1', '[TIME]', '[IP]', '[TIME]', '1');";
	$sql = str_replace(
		array('[PREFIX]', '[NAME]','[PASS]', '[TIME]', '[IP]'),
		array($prefix, $admin['username'],$password, time(), get_client_ip(1)),
		$sql);
	$sql2="INSERT INTO `[PREFIX]system` (`name`,`value`) VALUES ('administrator','[NAME]' );";
	$sql2 = str_replace(
		array('[PREFIX]', '[NAME]','[PASS]', '[TIME]', '[IP]'),
		array($prefix, $admin['username'],$password, time(), get_client_ip(1)),
		$sql2);
	$db->execute($sql);$db->execute($sql2);
	show_msg('创始人帐号注册完成！');
}
